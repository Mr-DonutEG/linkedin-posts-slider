...
function move_post()
{
	...
	$current_index = array_search($id, array_column($rows, 'id'));

	if ($current_index !== false && (($direction === 'up' && $current_index > 0) || ($direction === 'down' && $current_index < count($rows) - 1))) {
		...
		$wpdb->update(
			$table_name,
			array('post_order' => $rows[$current_index]->post_order),
